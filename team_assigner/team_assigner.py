...
class TeamAssigner:
    ...
    def get_player_color(self,frame,bounding_box):
        ...
        top_half_img_BGR=image[int(image.shape[0]*(.4)):int(image.shape[0]*(.5)),
                               int(image.shape[1]*(.25)):int(image.shape[1]*(.75))]
            ...
            for player_id,track in player_tracks.items():
                if track['pitch_side']=='left':
                    if track['coord_tr'][0]>370:
                        continue
                    team_id=self.get_player_team(frames_left[frame_num],
                                                track['bbox'],
                                                player_id)
                    tracks['players'][frame_num][player_id]['team']=team_id
                    tracks['players'][frame_num][player_id]['team_color']=\
                        [int(i) for i in cv2.cvtColor(np.array([[[i for i in self.team_color_dict[team_id]]]],dtype=np.uint8),cv2.COLOR_Lab2RGB).tolist()[0][0]]
                else:
                    if track['coord_tr'][0]<430:
                        continue
                    team_id=self.get_player_team(frames_right[frame_num],
                                                track['bbox'],
                                                player_id)
                    tracks['players'][frame_num][player_id]['team']=team_id
                    tracks['players'][frame_num][player_id]['team_color']=\
