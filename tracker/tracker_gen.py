...
class Tracker:
    ...
    def detect_player(self,frames):
        ...
        for i in range(0,len(frames),batch_size):
            result=model_player.track(frames[i:i+batch_size],conf=0.8,persist=True)
            detections+=result
        return detections
    ...
    def tracks_generator(self,frames,read_stub=False,stub_path=None):
        tracker=sv.ByteTrack(frame_rate=10,track_activation_threshold=.5)
        ...
        for frame_num,detection in enumerate(player_detections):
            ...
            for frame_detection in detection_with_tracks:
                ...
                tracks['players'][frame_num][track_id]={'bbox':bounding_box,'coord_frame':coord_frame}
        # 공
        for frame_num,detection in enumerate(ball_detections):
            ...
            for frame_detection in detection_sv:
                ...
                if len(bounding_box)>0:
                    coord_frame=get_coordnate(bounding_box)
                    tracks['ball'][frame_num][1]={'bbox':bounding_box,'coord_frame':coord_frame}
    ...
    def concat_tracks(self,tracks_left,tracks_right):
        ...
        for frame_num,player_tracks in enumerate(tracks_left['players']):
            ...
            tracks_concat['players'][frame_num]['right']=tracks_right['players'][frame_num]
