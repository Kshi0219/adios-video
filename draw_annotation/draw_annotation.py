...
class annotator:
    ...
    def annotate(self,base_pitch,tracks):
        output_frames=[]
        for frame_num in range(len(tracks['players'])):
            frame=base_pitch.copy()
            player_dict=tracks['players'][frame_num]
            ball_dict=tracks['ball'][frame_num]

            # 필드플레이어 annotate
            for track_id,player in player_dict.items():
                player_color_lab=player.get('team_color',(0,0,255))
                '''player_color_rgb=[int(i) for i in cv2.cvtColor(
                    np.array([[[i for i in player_color_lab]]],dtype=np.uint8),
                    cv2.COLOR_Lab2RGB).tolist()[0][0]]'''
                frame=self.draw_ellipse(
                    frame,player['bbox'],tuple(player_color_lab),track_id)
